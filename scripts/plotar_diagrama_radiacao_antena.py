...
O script não estima o diagrama da APCA8090 a partir do ganho nominal. O modo
--example existe somente para validar a instalação e produz curvas ilustrativas.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--title",
        default="Diagrama de radiação relativo",
...
def load_measurements(path: Path) -> dict[str, list[tuple[float, float]]]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as stream:
        ...
        required = {"plano", "angulo_graus", "nivel_db"}
        ...
        for line_number, row in enumerate(reader, start=2):
            ...
            level_text = (row.get("nivel_db") or "").strip()
...
def plot(
    ...
    figure.text(
        0.5,
        -0.01,
        "Cada plano foi normalizado pelo próprio máximo (0 dB).",
        ha="center",
        fontsize=9,
    )
